---
name: mode
description: Set how aggressively the workflow confirms before acting — cautious, default, or autonomous. Stored at .claude/dd-mode and read by the SessionStart hook and /impl. With no argument, reports the current mode.
---
...
Mode is stored at `.claude/dd-mode` (project-local, one word). The SessionStart hook reads it to state the active mode in ambient context, and `/impl` restates it at the top of every run.
...
1. No argument: read `.claude/dd-mode` (treat absent as `default`) and print `Mode: <mode> — <one-line behavior>.` Stop.
...
- The only filesystem write is `.claude/dd-mode`. Touch nothing else.
- Consider adding `.claude/dd-mode` to `.gitignore` if the mode is a personal preference rather than a shared project setting.
