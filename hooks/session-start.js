#!/usr/bin/env node
// developersDevelopers SessionStart hook.
//
...
//
// Reads the active mode from <project>/.claude/dd-mode (written by /mode).
...
function readMode() {
  ...
  const modeFile = path.join(projectDir, ".claude", "dd-mode");
  try {
...
const context = [
  ...
  "",
  "Mark deliberate borderline routing calls in-code with a `// dD:` comment (`# dD:` for Python/shell) that names the upgrade trigger, e.g. `// dD: routed simple, escalate if sorting grows past this file`. /debt audits these.",
  "",
